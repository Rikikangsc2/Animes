...
async function fetchAnimeData() {
  try {
    return (await axios.get('http://nue-db.vercel.app/read/nuenime1')).data || [];
  } catch (error) {
    ...
    res.send(`
        ...
        <body>
            ...
            <div class="bottom-bar">
                ...
                <a href="#" onclick="showHistory()"><i class="fas fa-history"></i> History</a>
            </div>
            <script>
...
app.get('/stream', async (req, res) => {
  try {
    ...
    // Simpan history ke local storage
    const history = JSON.parse(localStorage.getItem('history') || '[]');
    history.unshift({
      url: req.url,
      title: `${anime.title} - Episode ${episode}`
    });
    localStorage.setItem('history', JSON.stringify(history.slice(0, 20))); // Simpan maksimal 20 history
